...
<svelte:head>
  ...
  <meta
    name="description"
    content="A browser terminal for Applied Energistics 2."
  />
</svelte:head>
...
<div class="app-shell">
  ...
  <main class="workspace" id="main-content">
    <section class="terminal-frame" aria-label="ME terminal">
      <header class="terminal-titlebar">
        <div>
          <span class="terminal-kicker"
            >{terminal.network?.owner ?? 'Network'}</span
          >
          <h1>
            {terminal.tab === 'inventory' ? 'Terminal' : 'Crafting status'}
          </h1>
        </div>
      ...
      <div class="terminal-surface">
        {#if data.upstreamError}
          <StatusNotice
            kind="error"
            title="AE2 is unavailable"
            message={data.upstreamError}
          />
  ...
  <footer class="build-footer">
    ...
    <a href="https://github.com/kuba6000/AE2-Web-Integration"
      >AE2 Web Integration</a
    >
  </footer>
</div>
